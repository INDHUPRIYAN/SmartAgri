...
export default function DiseaseDetection() {
  ...
  const sendToModel = async (uri) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setDiseaseResult(`Prediction: ${data.prediction}`);
      } else if (data.predicted_class) {
        setDiseaseResult(`${data.predicted_class} (${data.confidence}%)`);
      } else {
        setDiseaseResult("Unexpected response from server.");
      }
    } catch (err) {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {loading && (
        <ActivityIndicator size="large" color="#56ab2f" style={{ marginTop: 20 }} />
      )}
